# Второй файл - Аналитический журнал знаний

---
# Python
---
```python
# database
## Здесь содержатся данные программы
import psycopg2
from psycopg2 import Error
import os
from datetime import datetime
from pathlib import Path


class Database:
    def __init__(self):
        self.connection = None
        self.cursor = None
        self.connect()

    def connect(self):
        """Подключение к базе данных PostgreSQL"""
        try:
            self.connection = psycopg2.connect(
                host="localhost",
                database="knowledge_journal",
                user="postgres",  # замените на вашего пользователя
                password="1111",  # замените на ваш пароль
                port="5432"
            )
            self.cursor = self.connection.cursor()
            print("✅ Подключение к PostgreSQL установлено")
            return True
        except Error as e:
            print(f"❌ Ошибка подключения к PostgreSQL: {e}")
            return False

    def initialize_database(self):
        """Инициализация базы данных (создание таблиц если их нет)"""
        try:
            # Создание таблицы notes
            self.cursor.execute("""
                CREATE TABLE IF NOT EXISTS notes (
                    id SERIAL PRIMARY KEY,
                    title VARCHAR(255) NOT NULL,
                    file_path VARCHAR(500) UNIQUE NOT NULL,
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    category VARCHAR(100)
                )
            """)

            # Создание таблицы tags
            self.cursor.execute("""
                CREATE TABLE IF NOT EXISTS tags (
                    id SERIAL PRIMARY KEY,
                    name VARCHAR(100) UNIQUE NOT NULL
                )
            """)

            # Создание таблицы note_tags
            self.cursor.execute("""
                CREATE TABLE IF NOT EXISTS note_tags (
                    note_id INT REFERENCES notes(id) ON DELETE CASCADE,
                    tag_id INT REFERENCES tags(id) ON DELETE CASCADE,
                    PRIMARY KEY (note_id, tag_id)
                )
            """)

            # Создание таблицы activity_log
            self.cursor.execute("""
                CREATE TABLE IF NOT EXISTS activity_log (
                    id SERIAL PRIMARY KEY,
                    note_id INT REFERENCES notes(id) ON DELETE CASCADE,
                    event_type VARCHAR(20) CHECK (event_type IN ('CREATE', 'UPDATE', 'VIEW')),
                    event_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP
                )
            """)

            self.connection.commit()
            print("✅ Таблицы базы данных инициализированы")
            return True

        except Error as e:
            print(f"❌ Ошибка инициализации БД: {e}")
            return False

    def close(self):
        """Закрытие соединения с БД"""
        if self.cursor:
            self.cursor.close()
        if self.connection:
            self.connection.close()
            print("🔌 Соединение с БД закрыто")


if __name__ == "__main__":
    # Тестирование подключения
    db = Database()
    if db.connection:
        print("✅ База данных доступна")
        db.initialize_database()
        db.close()
    else:
        print("❌ Не удалось подключиться к БД")

---

# Работа с Markdown файлами
## file_manager
import psycopg2
from psycopg2 import Error
import os
from datetime import datetime
from pathlib import Path


class Database:
    def __init__(self):
        self.connection = None
        self.cursor = None
        self.connect()

    def connect(self):
        """Подключение к базе данных PostgreSQL"""
        try:
            self.connection = psycopg2.connect(
                host="localhost",
                database="knowledge_journal",
                user="postgres",  # замените на вашего пользователя
                password="1111",  # замените на ваш пароль
                port="5432"
            )
            self.cursor = self.connection.cursor()
            print("✅ Подключение к PostgreSQL установлено")
            return True
        except Error as e:
            print(f"❌ Ошибка подключения к PostgreSQL: {e}")
            return False

    def initialize_database(self):
        """Инициализация базы данных (создание таблиц если их нет)"""
        try:
            # Создание таблицы notes
            self.cursor.execute("""
                CREATE TABLE IF NOT EXISTS notes (
                    id SERIAL PRIMARY KEY,
                    title VARCHAR(255) NOT NULL,
                    file_path VARCHAR(500) UNIQUE NOT NULL,
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    category VARCHAR(100)
                )
            """)

            # Создание таблицы tags
            self.cursor.execute("""
                CREATE TABLE IF NOT EXISTS tags (
                    id SERIAL PRIMARY KEY,
                    name VARCHAR(100) UNIQUE NOT NULL
                )
            """)

            # Создание таблицы note_tags
            self.cursor.execute("""
                CREATE TABLE IF NOT EXISTS note_tags (
                    note_id INT REFERENCES notes(id) ON DELETE CASCADE,
                    tag_id INT REFERENCES tags(id) ON DELETE CASCADE,
                    PRIMARY KEY (note_id, tag_id)
                )
            """)

            # Создание таблицы activity_log
            self.cursor.execute("""
                CREATE TABLE IF NOT EXISTS activity_log (
                    id SERIAL PRIMARY KEY,
                    note_id INT REFERENCES notes(id) ON DELETE CASCADE,
                    event_type VARCHAR(20) CHECK (event_type IN ('CREATE', 'UPDATE', 'VIEW')),
                    event_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP
                )
            """)

            self.connection.commit()
            print("✅ Таблицы базы данных инициализированы")
            return True

        except Error as e:
            print(f"❌ Ошибка инициализации БД: {e}")
            return False

    def close(self):
        """Закрытие соединения с БД"""
        if self.cursor:
            self.cursor.close()
        if self.connection:
            self.connection.close()
            print("🔌 Соединение с БД закрыто")


if __name__ == "__main__":
    # Тестирование подключения
    db = Database()
    if db.connection:
        print("✅ База данных доступна")
        db.initialize_database()
        db.close()
    else:
        print("❌ Не удалось подключиться к БД")

---

# gui
## Интерфейс Tkinter
import tkinter as tk
from tkinter import ttk, scrolledtext


class KnowledgeJournalApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Аналитический журнал знаний")
        self.root.geometry("1200x800")

        # 1. Панель управления (Frame)
        self.create_control_panel()

        # 2. Список конспектов (Treeview)
        self.create_notes_list()

        # 3. Область редактирования (ScrolledText)
        self.create_editor_panel()

        # 4. Панель тегов (Frame с Label и Entry)
        self.create_tags_panel()

        # 5. Вкладка аналитики (Notebook)
        self.create_analytics_tab()

---

# analytics
## Аналитика и сбор статистики
class Analytics:
    def get_category_stats(self):
        """Количество конспектов по категориям"""
        query = """
        SELECT category, COUNT(*) as count 
        FROM notes 
        GROUP BY category 
        ORDER BY count DESC
        """

    def get_recent_activity(self, days=7):
        """Активность за последние N дней"""
        query = """
        SELECT DATE(event_time) as date, 
               COUNT(*) as activities
        FROM activity_log 
        WHERE event_time >= CURRENT_DATE - INTERVAL '%s days'
        GROUP BY DATE(event_time)
        ORDER BY date
        """

    def get_top_tags(self, limit=5):
        """Самые популярные теги"""
        query = """
        SELECT t.name, COUNT(nt.note_id) as usage_count
        FROM tags t
        JOIN note_tags nt ON t.id = nt.tag_id
        GROUP BY t.id
        ORDER BY usage_count DESC
        LIMIT %s
        """

---

# excel_exporter
## Экспорт в Excel

from openpyxl import Workbook
from openpyxl.chart import BarChart, Reference
import os


class ExcelExporter:
    def export_report(self, data, filename="exports/report.xlsx"):
        """Создание Excel файла с двумя листами"""

        # 1. Создаём рабочую книгу
        wb = Workbook()

        # 2. Лист "Данные" - таблица со статистикой
        ws_data = wb.active
        ws_data.title = "Данные"

        # 3. Добавляем данные
        for row in data['category_stats']:
            ws_data.append([row['category'], row['count']])

        # 4. Лист "Графики" - диаграммы
        ws_chart = wb.create_sheet("Графики")

        # 5. Создаём столбчатую диаграмму
        chart = BarChart()
        chart.title = "Конспекты по категориям"
        chart.x_axis.title = "Категории"
        chart.y_axis.title = "Количество"

        # 6. Добавляем данные в диаграмму
        data_ref = Reference(ws_data, min_col=2, min_row=1, max_row=len(data['category_stats']))
        cats_ref = Reference(ws_data, min_col=1, min_row=2, max_row=len(data['category_stats']))

        chart.add_data(data_ref, titles_from_data=True)
        chart.set_categories(cats_ref)

        # 7. Добавляем диаграмму на лист
        ws_chart.add_chart(chart, "A1")

        # 8. Сохраняем файл
        os.makedirs("exports", exist_ok=True)
        wb.save(filename)

---

# pdf_exporter
## Генерация PDF отчётов
from reportlab.lib.pagesizes import A4
from reportlab.platypus import SimpleDocTemplate, Table, TableStyle, Paragraph, Image
from reportlab.lib.styles import getSampleStyleSheet
from reportlab.lib import colors
import matplotlib.pyplot as plt


class PDFExporter:
    def export_pdf(self, data, filename="exports/report.pdf"):
        """Генерация PDF отчёта"""

        # 1. Создаём PDF документ
        doc = SimpleDocTemplate(filename, pagesize=A4)
        story = []

        # 2. Заголовок
        styles = getSampleStyleSheet()
        title = Paragraph("Аналитический отчёт по конспектам", styles['Title'])
        story.append(title)

        # 3. Таблица с метриками
        metrics_data = [
            ["Метрика", "Значение"],
            ["Всего конспектов", data['total_notes']],
            ["Категорий", len(data['category_stats'])],
            ["Тегов", data['total_tags']],
            ["Активность за 7 дней", data['activity_7_days']]
        ]

        table = Table(metrics_data)
        table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
            ('GRID', (0, 0), (-1, -1), 1, colors.black)
        ]))
        story.append(table)

        # 4. Создаём график с matplotlib
        fig = self.create_chart(data)
        chart_path = "exports/chart_temp.png"
        fig.savefig(chart_path)

        # 5. Добавляем график в PDF
        chart_img = Image(chart_path, width=400, height=300)
        story.append(chart_img)

        # 6. Список последних конспектов
        recent_title = Paragraph("Последние конспекты:", styles['Heading2'])
        story.append(recent_title)

        # 7. Генерируем PDF
        doc.build(story)

---

# main
## Точка входа, запуск приложения

import tkinter as tk
from tkinter import ttk, messagebox, scrolledtext
import os
import subprocess
import platform
from datetime import datetime
from pathlib import Path


class KnowledgeJournalApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Аналитический журнал знаний")
        self.root.geometry("1200x800")

        # Получаем абсолютные пути к нужным папкам
        self.base_dir = Path(__file__).parent  # Папка где лежит main.py
        self.project_root = self.base_dir.parent  # Папка "PythonProject15" или другая

        # Папки для файлов проекта
        self.notes_dir = self.project_root / "notes_md"
        self.exports_dir = self.project_root / "exports"

        # Альтернативно, можно указать абсолютный путь (для вашего случая)
        self.exports_dir = Path(r"C:\Users\Student\Desktop\21Is\Проект2,1\exports")
        self.notes_dir = Path(r"C:\Users\Student\Desktop\21Is\Проект2,1\notes_md")

        print(f"📁 Папка проекта: {self.project_root}")
        print(f"📁 Папка с заметками: {self.notes_dir}")
        print(f"📁 Папка для экспорта: {self.exports_dir}")

        # Создаем папки если их нет
        self.notes_dir.mkdir(exist_ok=True)
        self.exports_dir.mkdir(exist_ok=True)

        # Проверяем доступность папок
        self.check_folders()

        # Текущий выбранный конспект
        self.current_note = None
        self.current_note_id = None
        self.current_file_path = None

        # Создаем все компоненты
        self.create_control_panel()
        self.create_notes_list()
        self.create_editor_panel()
        self.create_tags_panel()
        self.create_analytics_tab()

        # Загружаем существующие файлы
        self.load_existing_notes()

    def check_folders(self):
        """Проверка доступности папок"""
        if not self.notes_dir.exists():
            print(f"⚠️ Папка notes_md не найдена, создаем...")
        if not self.exports_dir.exists():
            print(f"⚠️ Папка exports не найдена, создаем...")

        print(f"✅ Папки проверены:")
        print(f"   notes_md: {self.notes_dir} (существует: {self.notes_dir.exists()})")
        print(f"   exports: {self.exports_dir} (существует: {self.exports_dir.exists()})")

    def create_control_panel(self):
        """Панель управления с полями ввода и кнопками"""
        control_frame = ttk.LabelFrame(self.root, text="Управление конспектами", padding=10)
        control_frame.pack(fill="x", padx=10, pady=5)

        # Поле для названия
        ttk.Label(control_frame, text="Название:").grid(row=0, column=0, sticky="w", padx=5, pady=5)
        self.title_entry = ttk.Entry(control_frame, width=40)
        self.title_entry.grid(row=0, column=1, padx=5, pady=5)

        # Поле для категории
        ttk.Label(control_frame, text="Категория:").grid(row=0, column=2, sticky="w", padx=5, pady=5)
        self.category_entry = ttk.Entry(control_frame, width=20)
        self.category_entry.grid(row=0, column=3, padx=5, pady=5)

        # Кнопки управления
        buttons_frame = ttk.Frame(control_frame)
        buttons_frame.grid(row=1, column=0, columnspan=4, pady=10)

        ttk.Button(buttons_frame, text="Создать", command=self.create_note).pack(side="left", padx=5)
        ttk.Button(buttons_frame, text="Сохранить", command=self.save_note).pack(side="left", padx=5)
        ttk.Button(buttons_frame, text="Удалить", command=self.delete_note).pack(side="left", padx=5)
        ttk.Button(buttons_frame, text="Просмотреть", command=self.view_note).pack(side="left", padx=5)
        ttk.Button(buttons_frame, text="Экспорт в Excel", command=self.export_to_excel).pack(side="left", padx=5)
        ttk.Button(buttons_frame, text="Экспорт в PDF", command=self.export_to_pdf).pack(side="left", padx=5)

        # Кнопка для открытия папки exports
        ttk.Button(buttons_frame, text="📂 Открыть папку exports",
                   command=self.open_exports_folder).pack(side="left", padx=5)

    def create_notes_list(self):
        """Список конспектов в Treeview"""
        list_frame = ttk.LabelFrame(self.root, text="Конспекты", padding=10)
        list_frame.pack(fill="both", expand=True, padx=10, pady=5)

        # Создаем Treeview
        columns = ("ID", "Название", "Категория", "Создан", "Обновлен", "Файл")
        self.notes_tree = ttk.Treeview(list_frame, columns=columns, show="headings", height=15)

        # Настраиваем колонки
        for col in columns:
            self.notes_tree.heading(col, text=col)
            if col == "ID":
                self.notes_tree.column(col, width=50)
            elif col == "Файл":
                self.notes_tree.column(col, width=300)
            else:
                self.notes_tree.column(col, width=150)

        # Добавляем скроллбар
        scrollbar = ttk.Scrollbar(list_frame, orient="vertical", command=self.notes_tree.yview)
        self.notes_tree.configure(yscrollcommand=scrollbar.set)

        # Упаковываем
        self.notes_tree.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")

        # Привязываем выбор элемента
        self.notes_tree.bind("<<TreeviewSelect>>", self.on_note_select)

    def create_editor_panel(self):
        """Область редактирования Markdown"""
        editor_frame = ttk.LabelFrame(self.root, text="Редактор Markdown", padding=10)
        editor_frame.pack(fill="both", expand=True, padx=10, pady=5)

        # Подсказки по синтаксису
        hint_frame = ttk.Frame(editor_frame)
        hint_frame.pack(fill="x", pady=(0, 10))

        ttk.Label(hint_frame, text="Подсказки:").pack(side="left", padx=5)
        hints = ["# Заголовок", "**жирный**", "*курсив*", "`код`", "```блок кода```"]
        for hint in hints:
            ttk.Label(hint_frame, text=hint, relief="ridge", padding=2).pack(side="left", padx=5)

        # Текстовый редакitor
        self.editor = scrolledtext.ScrolledText(editor_frame, wrap=tk.WORD, height=15)
        self.editor.pack(fill="both", expand=True)

    def create_tags_panel(self):
        """Панель для управления тегами"""
        tags_frame = ttk.LabelFrame(self.root, text="Теги", padding=10)
        tags_frame.pack(fill="x", padx=10, pady=5)

        # Поле для ввода нового тега
        input_frame = ttk.Frame(tags_frame)
        input_frame.pack(fill="x", pady=(0, 10))

        ttk.Label(input_frame, text="Новый тег:").pack(side="left", padx=5)
        self.tag_entry = ttk.Entry(input_frame, width=30)
        self.tag_entry.pack(side="left", padx=5)
        ttk.Button(input_frame, text="Добавить тег", command=self.add_tag).pack(side="left", padx=5)

        # Область для отображения текущих тегов
        self.tags_label = ttk.Label(tags_frame, text="Теги не выбраны", relief="sunken", padding=5,
                                    wraplength=1000, justify="left")
        self.tags_label.pack(fill="x")

    def create_analytics_tab(self):
        """Вкладка аналитики"""
        analytics_frame = ttk.LabelFrame(self.root, text="Аналитика и отчёты", padding=10)
        analytics_frame.pack(fill="x", padx=10, pady=5)

        # Frame для кнопок отчётов
        report_buttons_frame = ttk.Frame(analytics_frame)
        report_buttons_frame.pack(fill="x", pady=5)

        ttk.Button(report_buttons_frame, text="📊 Сформировать отчёт",
                   command=self.generate_report, width=20).pack(side="left", padx=5)

        ttk.Button(report_buttons_frame, text="📈 Создать Excel с графиками",
                   command=self.create_excel_with_charts, width=25).pack(side="left", padx=5)

        ttk.Button(report_buttons_frame, text="📄 Создать PDF отчёт",
                   command=self.create_pdf_report, width=20).pack(side="left", padx=5)

        # Область для вывода статистики
        stats_frame = ttk.Frame(analytics_frame)
        stats_frame.pack(fill="x", pady=10)

        # Статистика будет выводиться здесь
        self.stats_text = scrolledtext.ScrolledText(stats_frame, height=8, wrap=tk.WORD)
        self.stats_text.pack(fill="both", expand=True)
        self.stats_text.insert("1.0", "Статистика будет отображена здесь после формирования отчёта.")
        self.stats_text.config(state="disabled")

    # === ОСНОВНЫЕ МЕТОДЫ ===

    def load_existing_notes(self):
        """Загрузка существующих Markdown файлов из папки"""
        try:
            # Очищаем Treeview
            for item in self.notes_tree.get_children():
                self.notes_tree.delete(item)

            # Получаем список файлов в папке notes_md
            note_id = 1
            md_files = list(self.notes_dir.glob("*.md"))

            if not md_files:
                print("📝 Папка notes_md пуста, создайте первый конспект")
                return

            print(f"📚 Загружаю {len(md_files)} конспектов...")

            for file_path in md_files:
                try:
                    # Читаем первую строку для получения названия
                    with open(file_path, 'r', encoding='utf-8') as f:
                        first_line = f.readline().strip()

                    # Извлекаем название из заголовка Markdown
                    if first_line.startswith('# '):
                        title = first_line[2:].strip()
                    else:
                        title = file_path.stem

                    # Пытаемся извлечь категорию из файла
                    category = "Без категории"
                    tags_text = ""

                    with open(file_path, 'r', encoding='utf-8') as f:
                        content = f.read()
                        lines = content.split('\n')

                        for line in lines:
                            if line.lower().startswith("категория:"):
                                category = line.split(":", 1)[1].strip()
                            elif line.lower().startswith("теги:"):
                                tags_text = line.split(":", 1)[1].strip()

                    # Получаем дату изменения файла
                    mtime = os.path.getmtime(file_path)
                    mod_date = datetime.fromtimestamp(mtime).strftime("%Y-%m-%d %H:%M")

                    # Получаем дату создания файла
                    ctime = os.path.getctime(file_path)
                    create_date = datetime.fromtimestamp(ctime).strftime("%Y-%m-%d %H:%M")

                    # Добавляем в Treeview
                    self.notes_tree.insert("", "end", values=(
                        note_id,
                        title,
                        category,
                        create_date,
                        mod_date,
                        str(file_path.name)
                    ))

                    note_id += 1

                except Exception as e:
                    print(f"⚠️ Ошибка при загрузке файла {file_path}: {e}")

        except Exception as e:
            messagebox.showwarning("Внимание", f"Ошибка при загрузке файлов: {e}")

    def on_note_select(self, event):
        """Обработка выбора конспекта из списка"""
        selection = self.notes_tree.selection()
        if not selection:
            return

        item = self.notes_tree.item(selection[0])
        values = item["values"]

        if len(values) >= 5:
            self.current_note_id = values[0]
            self.current_note = values[1]

            # Заполняем поля
            self.title_entry.delete(0, tk.END)
            self.title_entry.insert(0, values[1])

            self.category_entry.delete(0, tk.END)
            if len(values) > 2:
                self.category_entry.insert(0, values[2])

            # Пытаемся загрузить содержимое файла
            try:
                # Ищем файл по названию
                file_name = f"{values[1]}.md"
                file_path = self.notes_dir / file_name

                # Если файл с таким именем не найден, ищем по имени из колонки "Файл"
                if not file_path.exists() and len(values) > 5:
                    alt_file_name = values[5]
                    file_path = self.notes_dir / alt_file_name

                if file_path.exists():
                    self.current_file_path = file_path
                    with open(file_path, 'r', encoding='utf-8') as f:
                        content = f.read()

                    self.editor.delete("1.0", tk.END)
                    self.editor.insert("1.0", content)

                    # Извлекаем теги из файла
                    tags = []
                    for line in content.split('\n'):
                        if line.lower().startswith("теги:"):
                            tags_line = line.split(":", 1)[1].strip()
                            tags = [tag.strip() for tag in tags_line.split(',')]
                            break

                    if tags:
                        self.tags_label.config(text=f"Теги: {', '.join(tags)}")
                    else:
                        self.tags_label.config(text="Теги не указаны")

                else:
                    messagebox.showwarning("Файл не найден",
                                           f"Файл конспекта не найден: {file_path}")
                    self.editor.delete("1.0", tk.END)
                    self.editor.insert("1.0", f"# {values[1]}\n\nКатегория: {values[2]}\n\n")
                    self.tags_label.config(text="Теги не выбраны")

            except Exception as e:
                messagebox.showerror("Ошибка", f"Не удалось загрузить файл: {e}")

    def create_note(self):
        """Создание нового конспекта"""
        title = self.title_entry.get().strip()
        category = self.category_entry.get().strip()

        if not title:
            messagebox.showwarning("Внимание", "Введите название конспекта!")
            return

        # Очищаем имя файла
        file_name = self.sanitize_filename(title) + ".md"
        file_path = self.notes_dir / file_name

        if file_path.exists():
            if not messagebox.askyesno("Файл существует",
                                       f"Файл '{file_name}' уже существует. Перезаписать?"):
                return

        try:
            # Получаем содержимое из редактора
            content = self.editor.get("1.0", tk.END).strip()

            # Если редактор пустой, создаем базовую структуру
            if not content:
                content = f"""# {title}

Категория: {category if category else "Без категории"}

Дата создания: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

## Содержание

## Основные идеи

## Выводы

Теги: """
            elif not content.startswith(f"# {title}"):
                # Добавляем заголовок и категорию, если их нет
                lines = content.split('\n')
                if not any(line.lower().startswith("категория:") for line in lines[:10]):
                    content = f"# {title}\n\nКатегория: {category}\n\n" + content

            # Создаем файл
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(content)

            # Генерируем ID
            new_id = len(self.notes_tree.get_children()) + 1
            current_date = datetime.now().strftime("%Y-%m-%d %H:%M")

            # Добавляем в Treeview
            self.notes_tree.insert("", "end", values=(
                new_id,
                title,
                category if category else "Без категории",
                current_date,
                current_date,
                file_name
            ))

            # Обновляем текущий конспект
            self.current_note_id = new_id
            self.current_note = title
            self.current_file_path = file_path

            messagebox.showinfo("Успех", f"Конспект '{title}' создан!\n\nФайл: {file_path}")

        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось создать конспект: {e}")

    def save_note(self):
        """Сохранение изменений в конспекте"""
        if not self.current_note:
            messagebox.showwarning("Внимание", "Выберите конспект для сохранения!")
            return

        title = self.title_entry.get().strip()
        category = self.category_entry.get().strip()

        if not title:
            messagebox.showwarning("Внимание", "Введите название конспекта!")
            return

        try:
            # Получаем содержимое из редактора
            content = self.editor.get("1.0", tk.END)

            # Определяем путь к файлу
            if self.current_file_path and self.current_file_path.exists():
                file_path = self.current_file_path
            else:
                file_name = self.sanitize_filename(title) + ".md"
                file_path = self.notes_dir / file_name

            # Обновляем категорию в содержимом
            lines = content.split('\n')
            updated_lines = []
            category_updated = False

            for line in lines:
                if line.lower().startswith("категория:"):
                    updated_lines.append(f"Категория: {category}")
                    category_updated = True
                else:
                    updated_lines.append(line)

            # Если категория не найдена, добавляем её
            if not category_updated:
                # Ищем строку после заголовка
                for i, line in enumerate(updated_lines):
                    if line.startswith('# '):
                        # Вставляем категорию после заголовка
                        updated_lines.insert(i + 1, f"Категория: {category}")
                        updated_lines.insert(i + 2, "")  # Пустая строка
                        break

            content = '\n'.join(updated_lines)

            # Сохраняем в файл
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(content)

            # Обновляем дату в Treeview
            selection = self.notes_tree.selection()
            if selection:
                current_date = datetime.now().strftime("%Y-%m-%d %H:%M")
                current_values = list(self.notes_tree.item(selection[0])["values"])

                if len(current_values) >= 5:
                    current_values[1] = title  # Название
                    current_values[2] = category  # Категория
                    current_values[4] = current_date  # Дата обновления

                    # Обновляем имя файла если изменилось название
                    old_file_name = current_values[5] if len(current_values) > 5 else ""
                    if old_file_name and title != self.current_note:
                        new_file_name = self.sanitize_filename(title) + ".md"
                        new_file_path = self.notes_dir / new_file_name

                        # Переименовываем файл
                        if file_path.exists():
                            os.rename(file_path, new_file_path)
                            file_path = new_file_path

                        current_values[5] = new_file_name

                    self.notes_tree.item(selection[0], values=tuple(current_values))

                    # Обновляем текущий конспект
                    self.current_note = title
                    self.current_file_path = file_path

            messagebox.showinfo("Успех", f"Конспект сохранен!\n\nФайл: {file_path}")

        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось сохранить конспект: {e}")

    def delete_note(self):
        """Удаление конспекта"""
        if not self.current_note:
            messagebox.showwarning("Внимание", "Выберите конспект для удаления!")
            return

        selection = self.notes_tree.selection()
        if not selection:
            return

        if messagebox.askyesno("Подтверждение",
                               f"Удалить конспект '{self.current_note}'?\n\nФайл также будет удален с диска."):
            try:
                # Удаляем файл
                if self.current_file_path and self.current_file_path.exists():
                    os.remove(self.current_file_path)
                    print(f"🗑️ Удален файл: {self.current_file_path}")

                # Удаляем из Treeview
                self.notes_tree.delete(selection[0])

                # Очищаем поля
                self.title_entry.delete(0, tk.END)
                self.category_entry.delete(0, tk.END)
                self.editor.delete("1.0", tk.END)
                self.tags_label.config(text="Теги не выбраны")

                # Сбрасываем текущий конспект
                self.current_note = None
                self.current_note_id = None
                self.current_file_path = None

                messagebox.showinfo("Успех", "Конспект удален!")

            except Exception as e:
                messagebox.showerror("Ошибка", f"Не удалось удалить конспект: {e}")

    def view_note(self):
        """Просмотр конспекта во внешнем приложении"""
        if not self.current_note:
            messagebox.showwarning("Внимание", "Выберите конспект для просмотра!")
            return

        # Определяем путь к файлу
        if self.current_file_path and self.current_file_path.exists():
            file_path = self.current_file_path
        else:
            # Пытаемся найти файл по названию
            file_name = self.sanitize_filename(self.current_note) + ".md"
            file_path = self.notes_dir / file_name

        if not file_path.exists():
            # Ищем любой файл с таким названием
            matching_files = list(self.notes_dir.glob(f"*{self.current_note}*.md"))
            if matching_files:
                file_path = matching_files[0]
            else:
                messagebox.showwarning("Внимание", "Файл конспекта не найден!")
                return

        try:
            # Открываем файл в ассоциированном приложении
            print(f"🔍 Открываю файл: {file_path}")

            if platform.system() == 'Windows':
                os.startfile(str(file_path))
            elif platform.system() == 'Darwin':  # macOS
                subprocess.run(['open', str(file_path)])
            else:  # Linux
                subprocess.run(['xdg-open', str(file_path)])

            print(f"✅ Файл открыт: {file_path}")

        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось открыть файл: {e}")

    def open_exports_folder(self):
        """Открытие папки exports в проводнике"""
        try:
            if platform.system() == 'Windows':
                os.startfile(str(self.exports_dir))
            elif platform.system() == 'Darwin':  # macOS
                subprocess.run(['open', str(self.exports_dir)])
            else:  # Linux
                subprocess.run(['xdg-open', str(self.exports_dir)])

            print(f"📂 Открыта папка: {self.exports_dir}")
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось открыть папку: {e}")

    def sanitize_filename(self, filename):
        """Очистка имени файла от недопустимых символов"""
        # Заменяем недопустимые символы
        invalid_chars = '<>:"/\\|?*'
        for char in invalid_chars:
            filename = filename.replace(char, '_')

        # Удаляем начальные и конечные точки/пробелы
        filename = filename.strip('. ')

        # Если имя пустое, используем "note"
        if not filename:
            filename = "note"

        return filename

    def add_tag(self):
        """Добавление тега к текущему конспекту"""
        tag = self.tag_entry.get().strip()
        if not tag:
            messagebox.showwarning("Внимание", "Введите тег!")
            return

        if not self.current_note:
            messagebox.showwarning("Внимание", "Выберите конспект для добавления тега!")
            return

        # Получаем текущее содержимое редактора
        content = self.editor.get("1.0", tk.END)
        lines = content.split('\n')

        # Ищем строку с тегами
        tag_line_index = -1
        for i, line in enumerate(lines):
            if line.lower().startswith("теги:"):
                tag_line_index = i
                break

        if tag_line_index >= 0:
            # Обновляем существующую строку с тегами
            current_tags = lines[tag_line_index].split(":", 1)[1].strip()
            if current_tags:
                new_tags = current_tags + f", {tag}"
            else:
                new_tags = tag
            lines[tag_line_index] = f"Теги: {new_tags}"
        else:
            # Добавляем новую строку с тегами в конец
            lines.append(f"Теги: {tag}")

        # Обновляем редактор
        new_content = '\n'.join(lines)
        self.editor.delete("1.0", tk.END)
        self.editor.insert("1.0", new_content)

        # Обновляем метку тегов
        current_tags_display = self.tags_label.cget("text")
        if current_tags_display == "Теги не выбраны":
            new_display = f"Теги: {tag}"
        else:
            new_display = current_tags_display + f", {tag}"
        self.tags_label.config(text=new_display)

        self.tag_entry.delete(0, tk.END)
        messagebox.showinfo("Успех", f"Тег '{tag}' добавлен!")

    def generate_report(self):
        """Генерация отчета"""
        self.stats_text.config(state="normal")
        self.stats_text.delete("1.0", tk.END)

        try:
            # Собираем статистику по файлам
            total_notes = len(self.notes_tree.get_children())
            categories = {}
            files_by_date = {}
            all_tags = set()

            # Анализируем файлы в папке
            for file_path in self.notes_dir.glob("*.md"):
                try:
                    mtime = os.path.getmtime(file_path)
                    mod_date = datetime.fromtimestamp(mtime)

                    # Группируем по дате изменения
                    date_str = mod_date.strftime("%Y-%m-%d")
                    files_by_date[date_str] = files_by_date.get(date_str, 0) + 1

                    # Читаем категорию и теги из файла
                    with open(file_path, 'r', encoding='utf-8') as f:
                        content = f.read()
                        lines = content.split('\n')

                        category = "Без категории"
                        for line in lines:
                            if line.lower().startswith("категория:"):
                                category = line.split(":", 1)[1].strip()
                                break

                        categories[category] = categories.get(category, 0) + 1

                        # Извлекаем теги
                        for line in lines:
                            if line.lower().startswith("теги:"):
                                tags_line = line.split(":", 1)[1].strip()
                                tags = [tag.strip() for tag in tags_line.split(',')]
                                all_tags.update(tags)
                                break

                except Exception as e:
                    print(f"⚠️ Ошибка при анализе файла {file_path}: {e}")

            # Выводим статистику
            stats_text = f"""📊 АНАЛИТИЧЕСКИЙ ОТЧЁТ
Дата генерации: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
Папка с заметками: {self.notes_dir}
Папка для экспорта: {self.exports_dir}
================================================

📈 ОБЩАЯ СТАТИСТИКА:
  • Всего конспектов: {total_notes}
  • Всего файлов в папке: {len(list(self.notes_dir.glob('*.md')))}
  • Размер папки notes_md: {self.get_folder_size(self.notes_dir):.2f} КБ
  • Уникальных тегов: {len(all_tags)}

📂 РАСПРЕДЕЛЕНИЕ ПО КАТЕГОРИЯМ:
"""
            for category, count in sorted(categories.items(), key=lambda x: x[1], reverse=True):
                percentage = (count / total_notes * 100) if total_notes > 0 else 0
                stats_text += f"  • {category}: {count} ({percentage:.1f}%)\n"

            # Анализ по датам
            recent_days = 7
            recent_files = []
            for date_str, count in sorted(files_by_date.items(), reverse=True):
                recent_files.append((date_str, count))
                if len(recent_files) >= recent_days:
                    break

            recent_count = sum(count for _, count in recent_files)

            stats_text += f"""
📅 АКТИВНОСТЬ:
  • Изменено за последние 7 дней: {recent_count} файл(ов)
  • Уникальных дат изменений: {len(files_by_date)}
  • Самые активные даты:"""

            for date_str, count in sorted(files_by_date.items(), key=lambda x: x[1], reverse=True)[:3]:
                stats_text += f"\n    - {date_str}: {count} файл(ов)"

            if all_tags:
                stats_text += f"""
🏷️ ИСПОЛЬЗУЕМЫЕ ТЕГИ:
  {', '.join(sorted(all_tags))}
"""

            stats_text += f"""
📋 ТОП-5 ПОСЛЕДНИХ ФАЙЛОВ:
"""
            # Получаем последние 5 файлов по дате изменения
            files_with_dates = []
            for file_path in self.notes_dir.glob("*.md"):
                mtime = os.path.getmtime(file_path)
                files_with_dates.append((file_path, datetime.fromtimestamp(mtime)))

            files_with_dates.sort(key=lambda x: x[1], reverse=True)

            for i, (file_path, mod_date) in enumerate(files_with_dates[:5], 1):
                size_kb = os.path.getsize(file_path) / 1024
                stats_text += f"  {i}. {file_path.name} ({mod_date.strftime('%d.%m.%Y')}, {size_kb:.1f} КБ)\n"

            stats_text += f"""
✅ Отчёт готов к экспорту!
Для создания файлов используйте кнопки ниже.
"""

            self.stats_text.insert("1.0", stats_text)

        except Exception as e:
            self.stats_text.insert("1.0", f"Ошибка при формировании отчёта: {str(e)}")

        self.stats_text.config(state="disabled")
        messagebox.showinfo("Отчёт", "Статистика собрана! Используйте кнопки для создания файлов.")

    def create_excel_with_charts(self):
        """Создание Excel файла с графиками"""
        try:
            excel_file = self.exports_dir / f"report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"

            # Проверяем наличие библиотеки
            try:
                import openpyxl
                from openpyxl import Workbook
                from openpyxl.chart import BarChart, Reference, PieChart

                print(f"📊 Создаю Excel файл: {excel_file}")

                # Создаем рабочую книгу
                wb = Workbook()

                # ===== Лист 1: Данные =====
                ws_data = wb.active
                ws_data.title = "Данные"

                # Заголовки
                headers = ["ID", "Название", "Категория", "Создан", "Обновлен", "Файл", "Размер (КБ)"]
                ws_data.append(headers)

                # Данные из Treeview
                total_size = 0
                category_data = {}

                for child in self.notes_tree.get_children():
                    values = self.notes_tree.item(child)["values"]
                    if len(values) >= 5:
                        # Получаем размер файла
                        file_name = values[5] if len(values) > 5 else f"{values[1]}.md"
                        file_path = self.notes_dir / file_name

                        size_kb = 0
                        if file_path.exists():
                            size_kb = os.path.getsize(file_path) / 1024
                            total_size += size_kb

                        # Собираем статистику по категориям
                        category = values[2] if len(values) > 2 else "Без категории"
                        category_data[category] = category_data.get(category, 0) + 1

                        # Добавляем строку
                        row_data = list(values[:min(6, len(values))])
                        if len(row_data) < 6:
                            row_data.extend([""] * (6 - len(row_data)))
                        row_data.append(f"{size_kb:.1f}")
                        ws_data.append(row_data)

                # Итоговая строка
                ws_data.append([])
                ws_data.append(["ИТОГО:", "", "", "", "", f"Файлов: {len(self.notes_tree.get_children())}",
                                f"Размер: {total_size:.1f} КБ"])

                # ===== Лист 2: Статистика =====
                ws_stats = wb.create_sheet("Статистика")

                # Заголовки статистики
                ws_stats.append(["Категория", "Количество", "Процент"])

                # Данные по категориям
                total_notes = len(self.notes_tree.get_children())
                for category, count in sorted(category_data.items(), key=lambda x: x[1], reverse=True):
                    percentage = (count / total_notes * 100) if total_notes > 0 else 0
                    ws_stats.append([category, count, f"{percentage:.1f}%"])

                # Создаем столбчатую диаграмму
                chart1 = BarChart()
                chart1.title = "Конспекты по категориям"
                chart1.x_axis.title = "Категории"
                chart1.y_axis.title = "Количество"

                data_ref = Reference(ws_stats, min_col=2, min_row=1, max_row=len(category_data) + 1)
                cats_ref = Reference(ws_stats, min_col=1, min_row=2, max_row=len(category_data) + 1)

                chart1.add_data(data_ref, titles_from_data=True)
                chart1.set_categories(cats_ref)

                ws_stats.add_chart(chart1, "E2")

                # ===== Лист 3: Графики =====
                ws_charts = wb.create_sheet("Графики")

                # Круговая диаграмма
                chart2 = PieChart()
                chart2.title = "Распределение по категориям"

                data_ref = Reference(ws_stats, min_col=2, min_row=2, max_row=len(category_data) + 1)
                cats_ref = Reference(ws_stats, min_col=1, min_row=2, max_row=len(category_data) + 1)

                chart2.add_data(data_ref, titles_from_data=True)
                chart2.set_categories(cats_ref)

                ws_charts.add_chart(chart2, "B2")

                # Сохраняем файл
                wb.save(excel_file)

                messagebox.showinfo("Успех",
                                    f"Excel файл создан!\n\nПуть: {excel_file}\n\nОткройте папку exports чтобы посмотреть файл.")
                print(f"✅ Excel файл создан: {excel_file}")

                # Предлагаем открыть папку
                if messagebox.askyesno("Открыть папку", "Открыть папку с файлом?"):
                    self.open_exports_folder()

            except ImportError:
                messagebox.showwarning("Библиотека отсутствует",
                                       "Установите openpyxl: pip install openpyxl")

        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось создать Excel файл: {e}")
            print(f"❌ Ошибка при создании Excel: {e}")

    def create_pdf_report(self):
        """Создание PDF отчёта"""
        try:
            pdf_file = self.exports_dir / f"report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"

            print(f"📄 Создаю PDF файл: {pdf_file}")

            # Проверяем наличие библиотеки
            try:
                from reportlab.lib.pagesizes import A4
                from reportlab.pdfgen import canvas
                from reportlab.lib.units import cm
                from reportlab.lib.colors import HexColor

                # Создаем PDF
                c = canvas.Canvas(str(pdf_file), pagesize=A4)
                width, height = A4

                # Настройки
                margin = 2 * cm
                line_height = 14
                y = height - margin

                # ===== Заголовок =====
                c.setFont("Helvetica-Bold", 18)
                c.setFillColor(HexColor("#2c3e50"))
                c.drawString(margin, y, "АНАЛИТИЧЕСКИЙ ОТЧЁТ")
                y -= line_height * 2

                c.setFont("Helvetica", 10)
                c.setFillColor(HexColor("#7f8c8d"))
                c.drawString(margin, y, f"Журнал знаний - {datetime.now().strftime('%d.%m.%Y %H:%M')}")
                y -= line_height * 2

                # Линия
                c.setStrokeColor(HexColor("#3498db"))
                c.setLineWidth(1)
                c.line(margin, y, width - margin, y)
                y -= line_height * 2

                # ===== Общая информация =====
                c.setFont("Helvetica-Bold", 12)
                c.setFillColor(HexColor("#2c3e50"))
                c.drawString(margin, y, "ОБЩАЯ ИНФОРМАЦИЯ:")
                y -= line_height

                c.setFont("Helvetica", 10)
                c.setFillColor(HexColor("#2c3e50"))

                info_lines = [
                    f"• Всего конспектов: {len(self.notes_tree.get_children())}",
                    f"• Папка с заметками: {self.notes_dir}",
                    f"• Дата генерации: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}"
                ]

                for line in info_lines:
                    c.drawString(margin + 0.5 * cm, y, line)
                    y -= line_height

                y -= line_height

                # ===== Список конспектов =====
                c.setFont("Helvetica-Bold", 12)
                c.drawString(margin, y, "СПИСОК КОНСПЕКТОВ:")
                y -= line_height

                c.setFont("Helvetica", 9)
                headers = ["№", "Название", "Категория", "Обновлен"]
                col_widths = [1 * cm, 8 * cm, 4 * cm, 3 * cm]

                # Заголовки таблицы
                x = margin
                for header, col_width in zip(headers, col_widths):
                    c.drawString(x, y, header)
                    x += col_width

                y -= line_height
                c.setStrokeColor(HexColor("#bdc3c7"))
                c.line(margin, y, width - margin, y)
                y -= line_height

                # Данные таблицы
                c.setFont("Helvetica", 8)
                row_num = 1

                for child in self.notes_tree.get_children():
                    values = self.notes_tree.item(child)["values"]
                    if len(values) >= 3:
                        # Проверяем, помещаемся ли на странице
                        if y < margin + 3 * cm:
                            c.showPage()
                            y = height - margin
                            c.setFont("Helvetica", 8)

                        x = margin
                        row_data = [
                            str(row_num),
                            values[1] if len(values) > 1 else "",
                            values[2] if len(values) > 2 else "",
                            values[4] if len(values) > 4 else ""
                        ]

                        for i, (data, col_width) in enumerate(zip(row_data, col_widths)):
                            # Обрезаем длинный текст
                            if i == 1 and len(data) > 40:
                                data = data[:37] + "..."
                            c.drawString(x, y, data)
                            x += col_width

                        y -= line_height
                        row_num += 1

                        # Ограничиваем количество строк для первой страницы
                        if row_num > 15 and y > margin + 3 * cm:
                            break

                # ===== Статистика =====
                c.showPage()
                y = height - margin

                c.setFont("Helvetica-Bold", 12)
                c.setFillColor(HexColor("#2c3e50"))
                c.drawString(margin, y, "СТАТИСТИКА:")
                y -= line_height * 2

                # Собираем статистику
                categories = {}
                for child in self.notes_tree.get_children():
                    values = self.notes_tree.item(child)["values"]
                    if len(values) > 2:
                        category = values[2]
                        categories[category] = categories.get(category, 0) + 1

                c.setFont("Helvetica", 10)
                c.drawString(margin, y, "Распределение по категориям:")
                y -= line_height

                for category, count in sorted(categories.items(), key=lambda x: x[1], reverse=True):
                    percentage = (count / len(
                        self.notes_tree.get_children()) * 100) if self.notes_tree.get_children() else 0
                    c.drawString(margin + 0.5 * cm, y, f"• {category}: {count} ({percentage:.1f}%)")
                    y -= line_height

                y -= line_height

                # ===== Подвал =====
                c.setFont("Helvetica-Oblique", 8)
                c.setFillColor(HexColor("#95a5a6"))
                c.drawString(margin, margin / 2, f"Сгенерировано приложением 'Аналитический журнал знаний'")
                c.drawString(width - margin - 6 * cm, margin / 2, f"Страница 2 из 2")

                # Сохраняем PDF
                c.save()

                messagebox.showinfo("Успех",
                                    f"PDF отчёт создан!\n\nПуть: {pdf_file}\n\nОткройте папку exports чтобы посмотреть файл.")
                print(f"✅ PDF файл создан: {pdf_file}")

                # Предлагаем открыть папку
                if messagebox.askyesno("Открыть папку", "Открыть папку с файлом?"):
                    self.open_exports_folder()

            except ImportError:
                messagebox.showwarning("Библиотека отсутствует",
                                       "Установите reportlab: pip install reportlab")

        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось создать PDF: {e}")
            print(f"❌ Ошибка при создании PDF: {e}")

    def export_to_excel(self):
        """Экспорт простого Excel файла (для обратной совместимости)"""
        self.create_excel_with_charts()

    def export_to_pdf(self):
        """Экспорт простого PDF файла (для обратной совместимости)"""
        self.create_pdf_report()

    def get_folder_size(self, folder_path):
        """Получение размера папки в килобайтах"""
        total_size = 0
        for file_path in folder_path.rglob('*'):
            if file_path.is_file():
                total_size += file_path.stat().st_size
        return total_size / 1024

    def run(self):
        """Запуск приложения"""
        self.root.mainloop()


if __name__ == "__main__":
    app = KnowledgeJournalApp()
    app.run()
```

---

```sql
# create_tables
## SQL для создания таблиц
-- create_tables.sql

CREATE TABLE notes (
    id SERIAL PRIMARY KEY,
    title VARCHAR(255) NOT NULL,
    file_path VARCHAR(500) UNIQUE NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    category VARCHAR(100)
);

CREATE TABLE tags (
    id SERIAL PRIMARY KEY,
    name VARCHAR(100) UNIQUE NOT NULL
);

CREATE TABLE note_tags (
    note_id INT REFERENCES notes(id) ON DELETE CASCADE,
    tag_id INT REFERENCES tags(id) ON DELETE CASCADE,
    PRIMARY KEY (note_id, tag_id)
);

CREATE TABLE activity_log (
    id SERIAL PRIMARY KEY,
    note_id INT REFERENCES notes(id) ON DELETE CASCADE,
    event_type VARCHAR(20) CHECK (event_type IN ('CREATE', 'UPDATE', 'VIEW')),
    event_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Индексы для ускорения запросов
CREATE INDEX idx_notes_category ON notes(category);
CREATE INDEX idx_activity_time ON activity_log(event_time);
CREATE INDEX idx_activity_note ON activity_log(note_id);

```

---
# Python
---

---
# TXT
---

```txt
# requirements
## Инструкция по установке
psycopg2-binary==2.9.9        # Подключение к PostgreSQL
tkinter                       # GUI (обычно входит в стандартную библиотеку)
markdown==3.5.1               # Парсинг Markdown
openpyxl==3.1.2               # Работа с Excel
reportlab==4.0.4              # Генерация PDF
matplotlib==3.7.1             # Построение графиков
Pillow==10.1.0                # Работа с изображениями (для GUI)

```

---
# TXT
---
